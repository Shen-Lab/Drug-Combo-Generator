...
def sentence_to_token_prot(sentence, vocabulary,_WORD_SPLIT,MAX_size):
  UNK_ID = 3
  PAD_ID = 0
  words = []
...
def sentence_to_token_comp(sentence, vocabulary,_WORD_SPLIT,MAX_size):
  UNK_ID = 3
  PAD_ID = 0
  words = []
  for space_separated_fragment in sentence.strip().split():
...
def get_disease_info(data_path):

    vocab_size_compound=68
    vocab_size_protein=76
    prot_max_size = 152
    WORD_SPLIT_comp = re.compile(b"(\S)")
    WORD_SPLIT_prot = re.compile(b",")

    rev_vocab = []
    with open(data_path+"vocab_compound",'rb') as f:  
        rev_vocab.extend(f.readlines())
    rev_vocab = [tf.compat.as_bytes(line.strip()) for line in rev_vocab]
    vocab_comp = dict([(x, y) for (y, x) in enumerate(rev_vocab)])

    rev_vocab = []
    with open(data_path+"vocab_protein",'rb') as f:
        rev_vocab.extend(f.readlines())
    rev_vocab = [tf.compat.as_bytes(line.strip()) for line in rev_vocab]
    vocab_prot = dict([(x, y) for (y, x) in enumerate(rev_vocab)])
    gene_name = pickle.load( open( data_path+'genes_dict.pickle', "rb" ) )
    disease_feat = pd.read_csv(data_path+'z_d_mean.csv',header=None).to_numpy()
    disease_gene = np.array(pickle.load( open( data_path+'disease_gene_net.pickle', "rb" ) ))
    disease_gene_1hop = np.array(pickle.load( open( data_path+'disease_genes_1hop.pickle', "rb" ) ))
    num_dis = disease_gene_1hop.shape[0]
    num_genes = disease_gene_1hop.shape[1]
    gene_token = {}
    for k,v in gene_name.items():
        with open(data_path+'aaseqs/sps_'+str(k)+'.txt') as f:
             token = sentence_to_token_prot(tf.compat.as_bytes(f.readline().strip()),vocab_prot,WORD_SPLIT_prot,prot_max_size)
             gene_token[v] = token 
    ...
    for d in range(num_dis):
        ...
        for g in range(num_genes):
            if disease_gene_1hop[d,g] == 1:
               if disease_gene[d,g] == 1:
                  temp.append(count) 
               count += 1
               temp_tok.append(gene_token[g])

        disease_gene_list.append(temp)
        disease_1hop.append(temp_tok)

    return vocab_comp,disease_feat,disease_1hop,disease_gene_list 


def get_disease_name_info(data_path):
    ...
    vocab_prot = dict([(x, y) for (y, x) in enumerate(rev_vocab)])
    gene_name = pickle.load( open( data_path+'genes_dict.pickle', "rb" ) )
    gene_name_inv = dict([[v,k] for k,v in gene_name.items()])
    disease_feat = pd.read_csv(data_path+'z_d_mean.csv',header=None).to_numpy()
    disease_gene = np.array(pickle.load( open( data_path+'disease_gene_net.pickle', "rb" ) ))
    disease_gene_1hop = np.array(pickle.load( open( data_path+'disease_genes_1hop.pickle', "rb" ) ))
    num_dis = disease_gene_1hop.shape[0]
    ...
    for d in range(num_dis):
        ...
        for g in range(num_genes):
            if disease_gene_1hop[d,g] == 1:
               ...
               count += 1
               temp_tok.append(gene_token[g])
               temp_tok_name.append(gene_name_inv[g])


        disease_gene_list.append(temp)
        disease_1hop.append(temp_tok)
        disease_1hop_name.append(temp_tok_name)

    return vocab_comp,disease_feat,disease_1hop,disease_1hop_name,disease_gene_list
...
def load_conditional_drug_comb(type,path):
    if type=='d1':
        path = path + '/d1_sdf.txt'
        suppl = Chem.SDMolSupplier(path)
        data = []
        for mol in suppl:
            data.append(Chem.MolToSmiles(mol))


    elif type=='d2':
        path = path + '/d2_sdf.txt'
        suppl = Chem.SDMolSupplier(path)
        data = []
        for mol in suppl:
            data.append(Chem.MolToSmiles(mol))
...
class MoleculeEnv(gym.Env):
    ...
    def init(self,path,vocab_comp,disease_feat,disease_1hop,disease_gene_list,data_type='zinc',logp_ratio=1, qed_ratio=1,sa_ratio=1,reward_step_total=1,is_normalize=0,reward_type='gan',reward_target=0.5,has_scaffold=False,has_feature=False,is_conditional=False,conditional='d1',max_action=128,min_action=20,force_final=False):
        '''
        ...
        self.vocab_comp = vocab_comp
        self.disease_feat = disease_feat
        self.disease_1hop = disease_1hop
        self.disease_gene_list = disease_gene_list
        self.disease_id = 0

        self.conditional_list = load_conditional_drug_comb(conditional,path)
        
        print("conditional: " + conditional)
        print("coditional list:")
        print(self.conditional_list)
    ...
    def step(self, action):
        ...
        if action[0,3]==0 or self.counter < self.min_action: # not stop
            stop = False
            if action[0, 1] >= total_atoms and action[0, 1] - total_atoms < len(self.possible_atom_types) :
                self._add_atom(action[0, 1] - total_atoms)  # add new node
                action[0, 1] = total_atoms  # new node id
                self._add_bond(action)  # add new edge
            elif action[0, 1] >= total_atoms and action[0, 1] - total_atoms >= len(self.possible_atom_types) : 
                stop = True
                action[0,3] = 0
            else:
                self._add_bond(action)  # add new edge
        ...
        if self.check_valency():
            if self.mol.GetNumAtoms()+self.mol.GetNumBonds()-self.mol_old.GetNumAtoms()-self.mol_old.GetNumBonds()>0:
                #print("successfully add node/edge")
        ...
        if terminate_condition or self.force_final:
            ...
            info['smile_code'] = self.get_final_smiles_code()
            #if self.is_conditional:
        ...
        return ob,reward,new,info,np.expand_dims(self.disease_feat[self.disease_id,],axis=0),self.disease_1hop[self.disease_id], self.disease_gene_list[self.disease_id]



    def reset(self,disease_id,smile=None):
        '''
        ...
        self.counter = 0
        ob = self.get_observation()
        return ob,np.expand_dims(self.disease_feat[self.disease_id,],axis=0),self.disease_1hop[self.disease_id], self.disease_gene_list[self.disease_id]
    ...
    def _add_bond(self, action):
        ...
        :return:
        '''
        if int(action[0,0]) == int(action[0,1]):
           #print("similar node for an edge")
           return False
    ...
    def get_final_smiles_code(self):
        """
        Returns a SMILES of the final molecule. Converts any radical
        electrons into hydrogens. Works only if molecule is valid
        :return: SMILES
        ...
        token = sentence_to_token_prot(tf.compat.as_bytes(smi),self.vocab_comp,WORD_SPLIT_comp,drug_max_size)
        return token
    ...
    def get_expert(self, batch_size,disease_id,is_final=False,curriculum=0,level_total=6,level=0):
        ob = {}
        atom_type_num = len(self.possible_atom_types)
        bond_type_num = len(self.possible_bond_types)
        ...
        return ob,ac,np.tile(np.expand_dims(self.disease_feat[self.disease_id,],axis=0),[batch_size,1])
